...
ploidy <- read.delim("1_purity_ploidy/ploidy.tsv", 
                     stringsAsFactors = F)
...
for (ii in seq_along(samples)) {
  ...
  peifer_file_tmp = dir("data/peifer/Mutation_Clustering/", pattern
                        = unlist(strsplit(samplename, split = "-"))[1], full.names = T)
  
  peifer_file_assignments = peifer_file_tmp[1]
  peifer_file_clusters = peifer_file_tmp[3]
  
  sahinalp_file = paste("data/sahinalp/citup_pilot63/samples_v1/", samplename, "_cluster_membership.txt", sep="")
  markowetz_file =  paste0("data/markowetz/clustering_pipeline_markowetz/", samplename, 
                           "/", samplename, "_assignment_probability_table.txt.gz")
  ...
  # markowetz_file = toString(args[6])
  ...
  if (file.exists(peifer_file_assignments)) {
    peifer = parse.mut.assignments.peifer(peifer_file_assignments)
  } else {
    peifer = NULL
  }
  ...
  if(file.exists(markowetz_file)) {
    markowetz = parse.mut.assignments(markowetz_file)
  } else {
    markowetz = NULL
  }
  
  vector_of_names = c("morris", "vanloo_wedge", "sahinalp", "markowetz", "peifer")
  list_of_tables = list(morris, vanloo_wedge, sahinalp, markowetz, peifer)
  
  #######################################################################
  # Get which mutations were assigned by whome
  #######################################################################
  get.mut.assigned.by.who = function(master_table, list_of_tables, vector_of_names) {
    colnames(master_table)[1:2] = c("Chromosome",  "Position")
    for (i in 1:length(list_of_tables)) {
      ...
      if (!is.null(sub)) {
        ...
        # ke: problem with finding the union e.g method1: chr 16  pos 6020374;  method2: chr 18  pos 6020374 
        # solution: use dplyr left_join
        ...
        #       
        colnames(sub)[1:2] = c("Chromosome",  "Position")
        master_table = left_join(master_table, sub[,c(1:3)], by = c("Chromosome"="Chromosome", 
                                                                    "Position"="Position"))
        master_table[,ncol(master_table)] = !is.na(master_table[,ncol(master_table)])
      } else {
...
#   if(!is.null(vanloo_wedge)) {
#     master_table = vanloo_wedge[,c(1,2)]
#   } else if (!is.null(peifer)){
#     master_table = peifer[,c(1,2)]
#   } else if (!is.null(markowetz)) {
#     master_table = markowetz[,c(1,2)]
#   } else if  (!is.null(morris)) {
#     master_table = morris[,c(1,2)]
#   } else {
#     master_table = sahinalp[,c(1,2)]
#   }
...
#   }
  ...
  print("Get which mutations were assigned by whome")
  #######################################################################
  ...
  print("Synchronise mutation assignment tables")
  ...
  print("Calculate similarities between each pair of matrices")
  ...
  print("Plot a heatmap with data ordered according to the first method, if no results use the second")
  ...
  for (i in (startpoint+1):length(dat.shared)) {
    if (!is.null(dat.shared[[i]]) & nrow(dat.shared[[i]]) > 0) {
      # ke: problems with single cluster call in orginal code
      
      if (dim(dat.shared[[i]])[2] == 4) {
        most.likely.node.assignments = cbind(most.likely.node.assignments, dat.shared[[i]][,3:(ncol(dat.shared[[i]])-1)])
      } else {
  ...
  print("Save a table with all most likely assignments")
  ...
  print("Obtain cluster locations by taking the mean Subclonal fraction of its assigned mutations")
  cat("\n", "finished sample", ii, "in", length(samples), "\n", file = "2_mutation_assignments_debug_progress", append = T)
}
